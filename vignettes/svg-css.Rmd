...
Recent versions of librsvg can also apply an external CSS file to the SVG. Hence we can split the drawing semantics from the styling. The external "css" paramter requires a somewhat recent version of librsvg so it may not work on older Linux systems. But it works for sure on any MacOS or Windows, and the latest versions of Ubuntu, Debian, Fedora, etc.
